/**
 * First-run wizard (design §09): four 560×560 cards centred on the canvas.
 * Owns the flow store and drives the real setup actions; a thin footer handles
 * Back / Continue while each card keeps its own real primary action (configure
 * vault, download models, finish).
 *
 * Finish is gated on the ENGINE truth: the required keys must actually validate
 * (a real call), the vault must be configured, and the models must be present —
 * nothing here is faked to unlock the button.
 */
...
export interface OnboardingActions {
  ...
  beginGoogleConnect: (store: OnboardingFlowStore) => Promise<void>;
  finishOnboarding: (
...
const LIVE_ACTIONS: OnboardingActions = {
  ...
  beginGoogleConnect: (store) => beginGoogleConnect(store),
...
export function OnboardingWizard({
  ...
  // Stable fallbacks — never call hooks in default params (rules of hooks).
  ...
  // Mount: pre-fill from real setup.status and wire the streaming events.
  useEffect(() => {
      ...
      <OnboardingCardFrame step={step}>
        ...
        {step === 4 && (
          <StepModels
            ...
            onDownload={() => void actions.beginModelDownload(flowStore)}
            onConnectGoogle={() => void actions.beginGoogleConnect(flowStore)}
        ...
        <div className="flex items-center gap-[var(--space-4)]" style={{ width: 560 }}>
          ...
          {step < 4 && (
            <OmniButton
              variant="secondary"
              className="ml-auto"
              disabled={step === 2 && !vaultConfigured}
...
/** A concise, plain-voice explanation of what still blocks Finish. */
