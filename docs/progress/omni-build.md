...
| m2-enhance | enhance pipeline, templates, extraction, finalization, auto-stop | engine/enhance, server wiring, UI meetings swap | DONE on wip branch — resumed agent kept ALL predecessor partials (verified against real APIs), fixed 2 real bugs found by new tests (truncation-marker count inexact; newline title made a meeting permanently unfinalizable), added auto-stop + server wiring + live Library detail pane; ~180 new tests, repo 1126 green, pnpm 508 green, live smoke passed. Follow-up queued: live-meeting stop-flow should offer meeting.finalize (Library 'Enhance now' works today); M4 cards read extraction_results.payload_json. |
| north-star #3 | read-only review | (read-only) | DONE — 4 GREEN / 2 AMBER (both bookkeeping: stale gate-state section + one 303-line test; fixed same session) |
| dictation-excellence | Wispr-Flow raise: universal injection, cleanup, dictionary, speed | engine/dictation extend, routing_table row, pill, src-tauri | RUNNING |
| m4-hands | approval cards, 5 tools (draft-only Gmail), Google OAuth vs fakes, audit | engine/agents, engine/google, migrations/0008, approval UI | RUNNING |
| wiring-pass | connect ask/detect/dictation server surfaces + finalize UX + devices.list | server.py, ws handler, live screen | RUNNING |
